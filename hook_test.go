...
func TestHook_WithPkgErrors(t *testing.T) {
	...
	for i, _ := range expectedStack {
		if stack[i].Function != expectedStack[i].Function {
			t.Errorf("expected method %v, got %v", expectedStack[i].Function, stack[i].Function)
		}
		if !strings.HasSuffix(stack[i].File, expectedStack[i].File) {
			t.Errorf("expected filename %v, got %v", expectedStack[i].File, stack[i].File)
		}
